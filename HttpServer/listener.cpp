...
QString Listener::send_not_found(int sockfd) {
    // 不知道发送啥，404吧
...
void Listener::process(int sockfd, sockaddr_in addr) {
    QString send_msg = QString("client address: ") +
    ...
    qDebug() << QString::fromStdString(recv_data) << "\n";
    ...
    if (std::regex_search(headers, match_result, reg_req)) {
        ...
        send_msg.append(QString::fromStdString(match_result[0].str()) + "\n");
    } else {
        // no status line found
        return;
    }

    send_msg.append(send_file(sockfd, root + uri) + "\n");
    shutdown(sockfd, SHUT_RDWR);
    emit send_message(send_msg);
}

QString Listener::send_file(int sockfd, const std::string &file)
{
    std::string filetype;
    if (file.find('.') != std::string::npos) {
        filetype = file.substr(file.find_last_of('.'));
    }
    // send binary stream
    std::ifstream in(file, std::ios_base::binary | std::ios_base::in);
    if (!in.is_open()) {
        // no file
        return send_not_found(sockfd);
    } else {
        // read the file binary stream
        ...
        // create buff
        std::unique_ptr<char, std::default_delete<char[]>> buf(new char[file_len]);
        in.read(buf.get(), file_len);
        in.close();
        ...
        // send header
        send(sockfd, response_header.c_str(), response_header.length(), 0);
        // send content
        send(sockfd, buf.get(), file_len, 0);
